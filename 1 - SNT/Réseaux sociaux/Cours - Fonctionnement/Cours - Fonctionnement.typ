...
#let repères_historiques = cadre(titre: "Repères historique")[
    #table(
        ...
        columns: (auto, 1fr),
        [*1995*],
        "Naissance d'un des premier réseaux sociaux, Classmates, qui met en relation d'anciens camarades de classe.",
        ...
        [*2010*],
        "Multiplication du partage des photos et des vidéos via l'entrée de nouveaux acteurs comme Instagram et Snapchat.",
    )
]
...
#let figure_identité_numérique = align(
    center,
    cetz.canvas({
        ...
        let texte_ligne(
            ...
            content(
                ...
                rect(
                    stroke: none,
                    inset: 3pt, /*(top: 0pt,bottom:6pt, rest:3pt)*/
                    text(size: 10pt, weight: "bold", titre),
                ),
            )
            content(
                "ligne.end",
                anchor: "north-" + orientation,
                rect(stroke: none, inset: 2pt /*(top: 6pt, bottom: 5pt,rest:2pt)*/, text(size: 8pt, sous_titre)),
            )
        }
        ...
        // Gauche
        texte_ligne(
            départ: (-décallage_départ, interligne * 2),
            arrivée: (-longueur, (séparation_verticale + interligne) * 2),
            distance_coude: -coude_base,
            titre: "Réputation",
            sous_titre: "ce que l'on dit sur moi",
        )

        texte_ligne(
            départ: (-décallage_départ, interligne),
            arrivée: (-longueur, séparation_verticale + interligne),
            distance_coude: -(coude_base + interligne),
            titre: "Expression",
            sous_titre: "ce que je dis",
        )

        texte_ligne(
            départ: (-décallage_départ, 0),
            arrivée: (-longueur, 0),
            distance_coude: -coude_base,
            titre: "Réseau",
...
#grid(
    columns: 2,
    ex1gauche,
    [#rect(inset: 3pt, outset: -3pt, stroke: 1pt, image("loginsocial2.png")) #v(-1em) #align(
            center,
            text(style: "italic", "Document: Interface de connection du site"),
        )],
)
